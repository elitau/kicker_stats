module MatchesHelper
  ...
  module ViewAttributes
    def caption
      yellow = "<span style=\"background-color: yellow\">#{yellow_players.collect(&:username).join(", ")}</span>"
      yellow_goals = "<span style=\"background-color: yellow\">#{self.yellow_goals}</span>"
      "#{yellow} vs. #{white_players.collect(&:username).join(", ")}: #{yellow_goals} zu #{self.white_goals}"
    end
  end
